...
{
    //release date 2012, C# changes the paradigm. http://www.ecma-international.org/publications/files/ECMA-ST/Ecma-334.pdf
    public class Version5 : IVersion
    {
        public async void Test()
        {
            ...
            Task<D> task = this.Set<D>(d, cancellationToken, new MyProgress<long>());
            cancellationToken.Register(() => { Console.WriteLine("cancelled"); });
            source.Cancel();
            await task;
            if (task.IsCompleted) d = task.Result;
